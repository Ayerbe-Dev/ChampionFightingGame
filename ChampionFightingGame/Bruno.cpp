#include "Bruno.h"

Bruno::Bruno() {

}

Bruno::Bruno(Player* player) {
	this->id = player->id;
	this->player = player;
	chara_kind = CHARA_KIND_BRUNO;
	chara_name = "bruno";
	resource_dir = "resource/chara/bruno";
	object_int.resize(CHARA_BRUNO_INT_MAX, 0);
	object_float.resize(CHARA_BRUNO_FLOAT_MAX, 0.0);
	object_flag.resize(CHARA_BRUNO_FLAG_MAX, false);
	object_string.resize(CHARA_BRUNO_STRING_MAX, "");
	status_script.resize(CHARA_BRUNO_STATUS_MAX, nullptr);
	enter_status_script.resize(CHARA_BRUNO_STATUS_MAX, nullptr);
	exit_status_script.resize(CHARA_BRUNO_STATUS_MAX, nullptr);

	load_fighter();
}

void Bruno::chara_main() {

}