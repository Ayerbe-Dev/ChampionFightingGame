...
void Fighter::process_fighter_pushbox_collisions(std::vector<Pushbox> pushboxes, std::vector<Pushbox> that_pushboxes) {
	...
	for (size_t i = 0, max = pushboxes.size(); i < max; i++) {
		for (size_t i2 = 0, max2 = that_pushboxes.size(); i2 < max2; i2++) {
			...
			bool p1_pos_change = add_pos_validate(glm::vec3(intersection * dir, 0.0f, 0.0f));
			bool p2_pos_change = that->add_pos_validate(glm::vec3(intersection * -dir, 0.0f, 0.0f));
	...
	for (size_t i = 0, max = pushboxes.size(); i < max; i++) {
		for (size_t i2 = 0, max2 = that_pushboxes.size(); i2 < max2; i2++) {
			if (is_rect_collide(pushboxes[i], that_pushboxes[i2])) {
				...
				float that_x_diff = that->object_float[BATTLE_OBJECT_FLOAT_X_SPEED];
				float pushbox_front = get_pos().x + get_pushbox_front(i);
				float pushbox_back = get_pos().x + get_pushbox_back(i);
				float that_pushbox_front = that->get_pos().x + that->get_pushbox_front(i2);
				float that_pushbox_back = that->get_pos().x + that->get_pushbox_back(i2);

				if (that_x_diff > 0.0 == that->get_pos().x < get_pos().x) {
					...
					if (facing_that) {
						...
						if (facing_same) {
							set_pos_validate(glm::vec3(that_pushbox_back - get_pushbox_front(i), get_pos().y, get_pos().z), true);
						}
						else {
							set_pos_validate(glm::vec3(that_pushbox_back - get_pushbox_back(i), get_pos().y, get_pos().z), true);
						}
					}
				}
...
bool Fighter::is_valid_incoming_fighter_hitbox_collision(Hurtbox* hurtbox, Hitbox* hitbox, Fighter* attacker) {
	...
	if (fighter_context == FIGHTER_CONTEXT_GROUND && hitbox->hit_result.chip_percent != -1.0) {
		bool reverse_block = ((hitbox->anchor.x > get_pos().x && hitbox->offset.x > get_pos().x)
			!= internal_facing_right) && ((attacker->get_pos().x > get_pos().x) != internal_facing_right);
...
void Fighter::process_definite_hitbox_activated(DefiniteHitbox* hitbox, Fighter* attacker) {
	...
	if (hit_move.target_frames) {
		object_float[BATTLE_OBJECT_FLOAT_X_SPEED] = (hit_move.target_x - get_pos().x) / hit_move.target_frames;
		object_float[BATTLE_OBJECT_FLOAT_Y_SPEED] = (hit_move.target_y - get_pos().y) / hit_move.target_frames;
	}
