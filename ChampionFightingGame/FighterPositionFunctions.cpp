...
bool Fighter::add_pos(vec3 pos, bool prev) {
	...
	float this_x_front = this->pos.x + (this->pos.x * facing_dir / 4);
	float that_x_front = that->pos.x + (that->pos.x * that->facing_dir / 4);
	...
	if (this->pos.x > WINDOW_WIDTH / 2) { //If you went out of the horizontal bounds
		if (prev) {
			...
			this->pos.x = WINDOW_WIDTH / 2 - pos.x;
		}
	...
	float this_x_back = this->pos.x + (this->pos.x * facing_dir / -2);
	float that_x_back = that->pos.x + (that->pos.x * that->facing_dir / -2);
	...
	float new_this_x_front = this->pos.x + (this->pos.x * facing_dir / 4);
	float new_that_x_front = that->pos.x + (that->pos.x * that->facing_dir / 4);
	...
	if (opponent_right != new_opponent_right && !fighter_flag[FIGHTER_FLAG_ALLOW_GROUND_CROSSUP] && situation_kind == FIGHTER_SITUATION_GROUND
		&& that->situation_kind == FIGHTER_SITUATION_GROUND && pos.x * facing_dir > 0) {
		this->pos.x = prev_pos.x;
		ret = false;
	}
...
bool Fighter::set_pos(vec3 pos, bool prev) {
	...
	float this_x_front = this->pos.x + (this->pos.x * facing_dir / 2);
	float that_x_front = that->pos.x + (that->pos.x * that->facing_dir / 2);
	bool opponent_right = this_x_front > that_x_front;
	this->pos = pos;
	if (this->pos.x + pos.x / 2 > WINDOW_WIDTH / 2) {
		if (prev) {
			this->pos.x = prev_pos.x;
		}
		else {
			this->pos.x = WINDOW_WIDTH / 2 - pos.x / 2;
		}
		ret = false;
	}
	if (this->pos.x - pos.x / 2 < WINDOW_WIDTH / -2) {
		if (prev) {
			...
			this->pos.x = WINDOW_WIDTH / -2 + this->pos.x / 2;
		}
		ret = false;
	}
	if (this->pos.y < 0) {
	...
	float new_this_x_front = this->pos.x + (this->pos.x * facing_dir / 2);
	float new_that_x_front = that->pos.x + (that->pos.x * that->facing_dir / 2);
	bool new_opponent_right = new_this_x_front > new_that_x_front;
