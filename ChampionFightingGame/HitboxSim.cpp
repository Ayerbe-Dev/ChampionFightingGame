...
void Simbox::update_rect(BattleObject* object) {
	if (prev_anchor != anchor) {
		prev_anchor = anchor;
		relative_anchor = anchor - glm::vec2(object->get_pos());
	...
	if (prev_offset != offset) {
		prev_offset = offset;
		relative_offset = offset - glm::vec2(object->get_pos());
	...
	if (prev_relative_anchor != relative_anchor) {
		prev_relative_anchor = relative_anchor;
		anchor = relative_anchor + glm::vec2(object->get_pos());
		prev_anchor = anchor;
	}
	if (prev_relative_offset != relative_offset) {
		prev_relative_offset = relative_offset;
		offset = relative_offset + glm::vec2(object->get_pos());
		prev_offset = offset;
	}
	if (object->prev_pos != object->get_pos()) {
		anchor = relative_anchor + glm::vec2(object->get_pos());
		prev_anchor = anchor;
		offset = relative_offset + glm::vec2(object->get_pos());
		prev_offset = offset;
